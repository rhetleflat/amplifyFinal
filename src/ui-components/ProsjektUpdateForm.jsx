...
export default function ProsjektUpdateForm(props) {
  const {
    id: idProp,
    prosjekt,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = prosjektRecord
      ? { ...initialValues, ...prosjektRecord }
      : initialValues;
    setProsjektNavn(cleanValues.prosjektNavn);
    setTimepris(cleanValues.timepris);
    setErrors({});
  };
  const [prosjektRecord, setProsjektRecord] = React.useState(prosjekt);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Prosjekt, idProp)
        : prosjekt;
      setProsjektRecord(record);
    };
    queryData();
  }, [idProp, prosjekt]);
  React.useEffect(resetStateValues, [prosjektRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Prosjekt.copyOf(prosjektRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ProsjektUpdateForm")}
      {...rest}
    >
      <TextField
        label="Prosjekt navn"
